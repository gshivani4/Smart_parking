...
cursor.execute('''
    CREATE TABLE IF NOT EXISTS User (
        ...
        vehicleNumber TEXT NOT NULL CHECK(vehicleNumber NOT LIKE '%[^a-zA-Z0-9]%')
...
#     vehicleNumber = StringField('vehicleNumber', validators=[DataRequired()])
...
class RegistrationForm(FlaskForm):
    ...
    #         raise ValidationError('Mobile number must be a 10-digit integer.')

    # def validate_aadhar(form, field):
    #     if not field.data.isdigit() or len(field.data) != 16:
    #         raise ValidationError('Aadhar number must be a 16-digit integer.')

    # def validate_vehicleNumber(form, field):
    #     if not field.data.isalnum():
    #         raise ValidationError('Vehicle number must not contain any symbols.')
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if(len(mobile)!=10 ):
            flash('Check Mobile number', 'error')
            return redirect('/register')
        
        if(len(aadhar)!=12 ):
            flash('Check Aadhar number', 'error')
            return redirect('/register')
        
        print("vehicleNumber : ",vehicleNumber)
        # Convert any lowercase characters to uppercase
        vehicleNumber = form.vehicleNumber.data.upper()

        # Check if the vehicle number contains only alphanumeric characters
        if not vehicleNumber.isalnum():
            flash('Vehicle number must contain only alphanumeric characters.', 'error')
            return redirect('/register')
        print("vehicleNumber : ",vehicleNumber)
        ...
        try:
            ...
            cursor.execute('INSERT INTO User (username, email, password,balance,aadhar,mobile,vehicleNumber) VALUES (?, ?, ?, ?, ?, ?, ?)', (username, email, password,0,aadhar,mobile,vehicleNumber))
            conn.commit()
...
def admin():
    ...
    cursor.execute('SELECT * FROM booking_details WHERE ? BETWEEN from_datetime AND to_datetime', (current_time.strftime('%Y-%m-%d %H:%M:%S'),))
    # booked_slot_details = [slot[0] for slot in cursor.fetchall()]
    booked_slot_details =[slot for slot in cursor.fetchall()]
    
    print("booked_slot_details : ",end="")
    print(booked_slot_details)

    close_db(conn)
    return render_template('admin.html', users=users, booked_slot_details=booked_slot_details)
        ...
        if user_id:
            ...
            if form.validate_on_submit():
                ...
                try:
                    ...
                    print("no previous saved data")
                if(balance<10):
                    flash("Low balance Please Recharge.", "error")
                    return redirect('/dashboard')
                try:
                    cursor.execute('INSERT INTO booking_details (user_id, username, place, slot_no, from_datetime, to_datetime) VALUES (?, ?, ?, ?, ?, ?)',
                                   (user_id, username, place, slot_no, from_datetime, to_datetime))
                    conn.commit()
                    ##update balance
                    cursor.execute('UPDATE user SET balance = ? WHERE user_id = ?', (balance-10, user_id))
                    conn.commit()
...
##new added buttons
    # Define route for viewing history
@app.route('/history')
def view_history():
    # Logic to fetch history data
    # Render history template with data
    conn = get_db()
    cursor = conn.cursor()

    cursor.execute('SELECT * FROM booking_details')
    history_details = cursor.fetchall()
    print("history_details :",history_details)
    close_db(conn)
    return render_template('history.html',history_details=history_details)

# Define route for recharging users
@app.route('/recharge')
def recharge_users():
    # Logic to fetch user data
    # Render recharge template with data
    conn = get_db()
    cursor = conn.cursor()
